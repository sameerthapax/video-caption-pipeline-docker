...
from services.client_pool import close_pooled_async_clients, get_judge_client, get_llm_client
...
logger = logging.getLogger("gemma-caption-pipe")
...
def main() -> None:
    ...
    try:
        ...
        logger.info(
            "Model routing: vision_caption=%s:%s judge=%s:%s",
            settings.llm_provider,
            _configured_model(settings.llm_provider),
            settings.judge_llm_provider,
            settings.judge_model or _configured_model(settings.judge_llm_provider),
        )
...
def _missing_generation_dependencies() -> list[str]:
    ...
    if settings.judge_llm_provider == "openrouter":
        if not settings.openrouter_proxy_url and not settings.openrouter_api_key:
            missing.append("OPENROUTER_PROXY_URL or OPENROUTER_API_KEY for judge")
        if not (settings.judge_model or settings.openrouter_model):
            missing.append("JUDGE_MODEL or OPENROUTER_MODEL")
    elif settings.judge_llm_provider == "fireworks":
        if not settings.fireworks_proxy_url and not settings.fireworks_api_key:
            missing.append("FIREWORKS_PROXY_URL or FIREWORKS_API_KEY for judge")
        if not (settings.judge_model or settings.fireworks_model):
            missing.append("JUDGE_MODEL or FIREWORKS_MODEL")
    else:
        missing.append("JUDGE_LLM_PROVIDER must be 'fireworks' or 'openrouter'")
    return missing
