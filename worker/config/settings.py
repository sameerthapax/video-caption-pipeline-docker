...
class Settings:
    def __init__(self) -> None:
        self.app_name = self._get_str("APP_NAME", "GemmaCaption-Pipe")
        ...
        self.worker_tmp_root = self._get_str("WORKER_TMP_ROOT", "/tmp/gemma-caption-pipe")

        self.debug_keep_temp = self._get_bool("DEBUG_KEEP_TEMP", False)
        self.max_concurrent_jobs = self._get_int("MAX_CONCURRENT_JOBS", 3)
        self.log_model_io = self._get_bool("LOG_MODEL_IO", False)

        self.llm_provider = self._get_str("LLM_PROVIDER", "openrouter")
        ...
        self.openrouter_model = self._get_str("OPENROUTER_MODEL", "google/gemma-4-31b-it")
        ...
        self.judge_llm_provider = self._get_str("JUDGE_LLM_PROVIDER", "fireworks")
        self.judge_model = self._get_str("JUDGE_MODEL", "accounts/fireworks/models/kimi-k2p6")
        self.fireworks_timeout_seconds = self._get_int("FIREWORKS_TIMEOUT_SECONDS", 120)
        self.fireworks_queue_timeout_seconds = self._get_int("FIREWORKS_QUEUE_TIMEOUT_SECONDS", 60)
        self.fireworks_max_retries = self._get_int("FIREWORKS_MAX_RETRIES", 2)
        ...
        self.fireworks_creative_temperature = self._get_float("FIREWORKS_CREATIVE_TEMPERATURE", 0.4)
        self.fireworks_vision_max_tokens = self._get_int("FIREWORKS_VISION_MAX_TOKENS", 20_000)
        self.fireworks_caption_max_tokens = self._get_int("FIREWORKS_CAPTION_MAX_TOKENS", 20_000)
        self.fireworks_judge_max_tokens = self._get_int("FIREWORKS_JUDGE_MAX_TOKENS", 20_000)
        self.caption_acceptance_threshold = self._get_float("CAPTION_ACCEPTANCE_THRESHOLD", 0.94)
